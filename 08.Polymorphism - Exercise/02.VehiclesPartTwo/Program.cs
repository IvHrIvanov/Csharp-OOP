    ...
    public class Program
    {
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                {
                    if (operation == nameof(bus.Drive))
                    {
                        bus.Drive(fuelOrKm);
                    }
                    else if (operation == nameof(bus.Refuel))
                    {
                        bus.Refuel(fuelOrKm);
                    }
                    else if(operation==nameof(bus.DriveEmpty))
                    {
                        bus.DriveEmpty(fuelOrKm);
                    }
            ...
            Console.WriteLine(bus.ReturnFuel());
